...
import { fieldOperatorsByType } from "../Context/FieldProperties";
...
const ApiGeneration = () => {
  const { list: engineConfigs } = useEngineConfigContext();
  const {
    fetchUserAttributes,
  ...
  const { save, saving } = useMyApiContext();
  ...
  useEffect(() => {
    if (["update", "delete", "view"].includes(selectedMethod)) {
      if (modelRecords.length === 0) {
        fetchModelRecords(currentModel);
      }
    }
  ...
  const toggleSelectedFields = (field, type) => {
    if (selectedFields.includes(field)) {
      ...
      setQBFields(QBFields.filter((f) => f.name !== field));
    } else {
      ...
      if (
        ...
        fetchRecords(fields[field].relation).then((response) => {
      ...
      <div className="flex flex-col items-center justify-start w-full h-full">
        ...
        <div className="lg:max-w-screen-md w-full">
          ...
          <div className="flex space-x-4 items-center">
            ...
            />
            {showingRetryButton ? (
              <PrimaryButton onClick={fetchModels} disabled={fetching}>
                {t("Retry")}
              </PrimaryButton>
            ) : (
              ""
