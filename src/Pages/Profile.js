...
import { API_GET_MODELS_URI, DEFAULT_CONFIG } from "../Context/Constants";
...
const Profile = () => {
  ...
  const { profile, updateOdooProfile, createOdooProfile, searchOdooProfile, user } =
    useAuthContext();
  ...
  useEffect(() => {
    let execute = async () => {
      await updateOdooProfile(profile.id, { x_userlanguage: currentLanguage[0] });
    };
    execute();
  });
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        if (profile) {
          await updateOdooProfile(profile.id, formValues);
          setSuccess("Profile updated successfully!");
        } else {
          await createOdooProfile(formValues);
          await searchOdooProfile(formValues.email);
          setSuccess("Profile created successfully!", true);
          navigate("/home");
        }
      } catch (e) {
        setError(e.message);
