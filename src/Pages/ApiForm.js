...
const ApiForm = () => {
  ...
  const { currentConfig } = useEngineConfigContext();
  ...
  useEffect(() => {
    fetchAllRecords(currentModel).then((res) => {
      setAllRecords(res);
  ...
  useEffect(() => {
    ...
    Promise.all(models.map((model) => fetchRecords(model))).then((all) => {
      setAllModelRecords(
        models.reduce(
          (prev, model, index) => ({
            ...prev,
