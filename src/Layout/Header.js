...
import { LANG_OPTIONS } from "../Context/Constants";
import { useAuthContext } from "../Context/AuthContext";
import PrimaryButton from "../Component/PrimaryButton";

const Header = ({ navItems }) => {
  const { setShowingSidebar } = useSidebarContext();
  const location = useLocation();
  const { loggedIn, logout, submitting, profile } = useAuthContext();
  const { t } = useTranslation("common");
  const { setSelectedLang, selectedLang } = useLanguageContext();
  const { switchToolTip, setSwitchToolTip } = useToolTipContext();

  useEffect(() => {
    if (profile) {
      setSelectedLang(profile.x_userlanguage);
    }
  }, []);
    ...
    <div className="w-full bg-white shadow-lg z-10 relative">
      <header className="flex px-6 py-4 text-blue-800 items-center">
        <div className="w-4/12">
          <button type="button" onClick={() => setShowingSidebar(true)} className="lg:hidden">
            <svg
              fill="#000000"
      ...
      {loggedIn ? (
        <nav className="border hidden lg:flex justify-between">
          ...
          <PrimaryButton onClick={logout} disabled={submitting}>
            {t("Logout")}
          </PrimaryButton>
        </nav>
      ) : (
